...
class Matrix { // matrix is column based
  ...
  void Write(char *filename) {
    ...
    for (i = 0; i < r; i++) {
      for (j = 0; j < c; j++) {
        value = (double)GetElement(j, i);
        fwrite(&value, 1, sizeof(double), f);
      }
    }
...
class Maths {
  ...
  public:
    ...
    double DotProductSSENotMultof4(double *v, double *v2, int n);

    void ZscoreSSE(double *data, double *mean, double *std, double *outputvect, int n);
};
